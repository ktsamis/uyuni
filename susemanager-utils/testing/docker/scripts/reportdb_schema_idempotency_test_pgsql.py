...
def find_next_version(schema_path):
    ...
    return (
        last_version.rsplit(".", 1)[0]
        + "."
        + str(int(last_version.rsplit(".", 1)[1]) + 1)
    )
...
def get_all_files(start, end, schema_path):
    # pylint: disable-next=consider-using-f-string
    start_schema = "uyuni-reportdb-schema-%s" % start
    # pylint: disable-next=consider-using-f-string
    end_schema = "uyuni-reportdb-schema-%s" % end
...
def create_fake_migration_path(schema_path, new_version, pr_file=None, version=None):
    ...
    fake_path = (
        schema_path
        # pylint: disable-next=consider-using-f-string
        + "/uyuni-reportdb-schema-%s-to-uyuni-reportdb-schema-%s/"
    ...
    for migration_file in files:
        ...
        fakedir = pcom.pop()
        # pylint: disable-next=consider-using-f-string
        fakedir = "uyuni-reportdb-schema-%s-to-uyuni-reportdb-schema-%s" % (
            last_version,
            new_version,
        )
        pcom.append(fakedir)
...
def dump_database(dump_name, excluded_tables=None):
    ...
    with open(sys.path[0] + "/clear-db-answers-pgsql.txt") as file:
        for line in file.read().splitlines():
            if line.split("=")[0] == "report-db-name":
...
def run_upgrade(upgrade_script, new_version):
    ...
    log_path = "/var/log/spacewalk/reportdb-schema-upgrade/"
    perl_libs = (
    ...
    command = (
        # pylint: disable-next=consider-using-f-string
        "export SUMA_TEST_SCHEMA_VERSION=%s; export PERLLIB=%s; %s -y --reportdb"
...
def argparser():
    ...
    if not os.path.isfile(args.upgrade_script):
        raise RuntimeError(
            # pylint: disable-next=consider-using-f-string
            "The file %s for the upgrade script does not exist"
            % args.upgrade_script
        )
