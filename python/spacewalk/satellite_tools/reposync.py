...
class KSDirHtmlParser(KSDirParser):
    def __init__(self, plug, dir_name):
        ...
        for s in (
            m.group(1) for m in re.finditer(r'(?i)<a href="(.+?)"', dir_html.decode())
        ):
            if not (
                re.match(r"/", s)
                or re.search(r"\?", s)
...
def set_filter_opt(option, opt_str, value, parser):
    ...
    if opt_str in ["--include", "-i"]:
        ...
        f_type = "-"
    parser.values.filters.append(
        (f_type, [v.strip() for v in value.split(",") if v.strip()])
    )
    ...
    for cert in (ca_cert, client_cert, client_key):
        ...
        if name is not None and pem is not None:
            ...
            cert_file = os.path.join(
                ssldir,
                # pylint: disable-next=consider-using-f-string
                "%s.pem" % re.sub(r"[^a-zA-Z0-9_-]", "_", name),
            )
...
class RepoSync(object):
    ...
    def _get_decompressed_file_checksum(abspath, hashtype):
        with fileutils.decompress_open(abspath) as src, tempfile.TemporaryFile(
            "w"
        ) as tmp:
    ...
    def import_groups(self, plug):
        ...
        if groupsfile:
            abspath = self.copy_metadata_file(
                plug, groupsfile, "comps", relative_comps_dir
            )
    ...
    def import_mediaproducts(self, plug):
        ...
        if mediaproducts:
            self.copy_metadata_file(
                plug, mediaproducts, "mediaproducts", relative_mediaproducts_dir
            )
    ...
    def import_packages(self, plug, source_id, url, is_non_local_repo):
        ...
        if (not self.filters) and source_id:
            ...
            filters = [
                (
                    row["flag"],
                    [v.strip() for v in row["filter"].split(",") if v.strip()],
                )
        ...
        with multiprocessing.Pool(
            processes=min(os.cpu_count() * 2, 32), maxtasksperchild=1
        ) as pool:
            results = [
                pool.apply_async(
            ...
            for i, result in enumerate(results):
                (
                    affected_channels_batch,
                    failed_packages_batch,
                    all_packages,
                    processed_batch,
                ) = result.get()
    ...
    def twisted_batch_indexes(self, total_size, batch_size):
        ...
        batches = [
            [
                batch_size * batch_index + element_index
                for element_index in range(0, batch_size)
            ]
        ...
        return [
            [
                twisted_batches[i][j]
                for j in range(0, batch_size)
                if twisted_batches[i][j] < total_size
            ]
            for i in range(0, batch_count)
        ]
    ...
    def import_package_batch(
        ...
        rhnSQL.initDB()
        h_delete_package_queue = rhnSQL.prepare(
    ...
    def import_kickstart(self, plug, repo_label, is_non_local_repo):
        ...
        while dirs_queue:
            ...
            if is_non_local_repo:
                ...
                parser = KSDirLocalParser(
                    plug.repo.urls[0].replace("file://", ""), cur_dir_name
                )
            for ks_file in parser.get_content():
                ...
                if (
                    not os.path.exists(os.path.join(mount_point, ks_path, repo_path))
        ...
        for addon_dir in treeinfo_parser.get_addons():
            ...
            repomd_file = plug.get_file(
                repomd_url, os.path.join(plug.repo.basecachedir, plug.name)
            )
                ...
                for i in repomd_xml.getElementsByTagName("data"):
                    if i.attributes["type"].value == "primary":
                        primary_url = str(
                            addon_dir
                            + "/"
                            + i.getElementsByTagName("location")[0]
                            .attributes["href"]
                            .value
                        )
                        break

                primary_zip = plug.get_file(
                    primary_url, os.path.join(plug.repo.basecachedir, plug.name)
                )
                if primary_zip:
                    ...
                    for i in xmldoc.getElementsByTagName("package"):
                        ...
                        if (
                            not os.path.exists(
                                os.path.join(mount_point, ks_path, repo_path)
                            )
                            or self.force_kickstart
                        ):
                            to_download.add(repo_path)
        ...
        if to_download:
            ...
            for item in to_download:
                params = {}
                plug.set_download_parameters(
                    params, item, os.path.join(mount_point, ks_path, item)
                )
                downloader.add(params)
            ...
            for item in to_download:
                ...
                insert_h.execute(
                    ...
                    checksum=getFileChecksum(
                        "sha256", os.path.join(mount_point, ks_path, item)
                    ),
                    st_size=st.st_size,
                    st_time=st.st_mtime,
                )
        ...
        if creds:
            ...
            if not credentials:
                log2(
                    ...
                    0,
                    "Could not figure out which credentials to use "
                    "for this URL: " + url.getURL(stripPw=True),
                    stream=sys.stderr,
                )
                sys.exit(1)
            ...
            if "extra_auth" in credentials and credentials["extra_auth"]:
                ...
                if not headers:
                    log2(
                        0,
                        0,
                        "Empty extra auth headers. Possibly the PAYG instance is down?",
                    )
    ...
    def upload_patches(self, notices):
        ...
        for notice in notices:
            ...
            e["advisory_type"] = errata_typemap.get(
                category, "Product Enhancement Advisory"
            )
    ...
    def import_products(self, repo):
        ...
        for product in products:
            ...
            if not row or "id" not in row:
                get_id_q = rhnSQL.prepare(
                    """SELECT sequence_nextval('suse_prod_file_id_seq') as id FROM dual"""
                )
                get_id_q.execute()
            ...
            h = rhnSQL.prepare(
                """select 1 from susePackageProductFile where package_id = :paid and prodfile_id = :prid"""
            )
            h.execute(paid=packrow["id"], prid=row["id"])
            ex = h.fetchone_dict() or None
            if not ex:
                h = rhnSQL.prepare(
                    """insert into susePackageProductFile (package_id, prodfile_id)
        ...
        for package in susedata:
            ...
            for keyword in package["keywords"]:
                ...
                if keyword in pkgkws:
                    ...
                    kadd = rhnSQL.prepare(
                        """INSERT INTO suseMdData (package_id, channel_id, keyword_id)
                                              VALUES(:package_id, :channel_id, :keyword_id)"""
                    )
                    kadd.execute(
                        package_id=pkgid,
                        channel_id=int(self.channel["id"]),
                        keyword_id=kwcache[keyword],
                    )
                    self.regen = True
            ...
            for label in pkgkws:
                if not pkgkws[label]:
                    ...
                    kdel = rhnSQL.prepare(
                        """DELETE FROM suseMdData WHERE package_id = :package_id
                                             AND channel_id = :channel_id AND keyword_id = :keyword_id"""
                    )
                    kdel.execute(
                        package_id=pkgid,
                        channel_id=int(self.channel["id"]),
                        keyword_id=kwcache[label],
                    )
                    self.regen = True
